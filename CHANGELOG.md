...
- **PROTO-PIN**: CI checkouts `ailib-official/ai-protocol` `29015b4` (PT-TTC-012 +
  PT-GEN-003; npm protocol still 1.2.0). Same pin class as ai-lib-rust #67.
...
- **Release automation:** `Publish (PyPI)` checkout now fetches tags (`fetch-depth: 0`, `fetch-tags: true`) so `git describe --exact-match` can detect `v*` tags after `workflow_run`.
